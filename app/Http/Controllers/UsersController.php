...
class UsersController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // ... atur kolom lainnya sesuai kebutuhan Anda
    ...
    public function vloadd(Request $request)
    {
        ...
        $filter = " WHERE 1=1 AND A.is_active='1' AND (A.role='1' OR A.role='2')";
        ...
        if ($search != '') {
            ...
            or A.branch_id like '%" . $search . "%' 
            ) ";
        }
        $sql = "SELECT A.id, A.first_name, A.email, B.name AS branch_id, A.role, A.is_verified 
            FROM users A  
...
public function vstoreinfo(Request $request)
{
    return "123";
    ...
    // Cari cabang berdasarkan country_id
    $branch = Branch::where('country_id', $country_id)->first();

    // Jika cabang ditemukan, tambahkan branch_id ke input
    if ($branch) {
        $input['branch_id'] = $branch->id;
    } else {
        $input['branch_id'] = null;
        // Handle the case where no branch is found for the given country
        // You may want to add appropriate error handling or default values
    }
    ...
    if ($request->ajax()) {
        ...
        return redirect()->route('user.vindex');
    }
}
